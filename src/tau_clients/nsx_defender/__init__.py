        ...
        if raw:
            ...
            if disposition and disposition.type == "attachment":
                ...
                return tau_clients.NamedBytesIO(
                    ...
                    # name=disposition.params.get("filename")
                    name=sha256_hash.hexdigest(),
                )
            else:
                return response.content
        else:
...
class PortalClient(AbstractClient):
    ...
    def get_tasks_from_knowledgebase(self, query_string: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        sorted_malscape_tasks = sorted(
            ...
            key=lambda x: datetime.datetime.strptime(x["submission"], self.DATETIME_FMT),
            reverse=True,
        )
    ...
    def get_result_artifact_names(
        ...
        for report in result.get("reports", []):
            if not report_types or any(x in report["report_versions"] for x in report_types):
                ...
                for metadata_item in temp.get("report", {}).get("analysis_metadata", []):
                    if not metadata_types or metadata_item["metadata_type"] in metadata_types:
                        ret.append(
                            {
                                "task_uuid": uuid,
                                "report_uuid": report["report_uuid"],
                                "artifact_name": metadata_item["name"],
                                "artifact_type": metadata_item["metadata_type"],
                            }
                        )
        return ret
...
class MultiDataCenterAnalysisClient(AnalysisClient):
    """
    This is a special 'AnalysisClient' that acts as a normal 'AnalysisClient' object unless
    the attribute 'available_clients' has been correctly initialized (via 'from_conf'):
        - If correctly initialized the attribute 'available_clients' contains a dictionary of
            other 'MultiDataCenterAnalysisClient' objects (including a proxy reference to itself)
            covering all the data centers.
        - If NOT correctly initialized (via the constructor for example), the instance behaves
            like a normal 'AnalysisClient' object.
    """

    @staticmethod
    def __call_super_method(
        instance: Any,
        name: str,
        *args: Any,
        **kwargs: Any,
        ...
        :param MultiDataCenterAnalysisClient|ProxyType instance: the instance
        ...
        """
        return getattr(super(MultiDataCenterAnalysisClient, instance), name)(*args, **kwargs)

    def __call_everywhere(self, method: Callable, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        """
        This method calls the parent method of 'func' for each client available.
        ...
        """
        return {
            data_center: self.__call_super_method(client, method.__name__, *args, **kwargs)
            for data_center, client in self.available_clients.items()
        }

    def __call_local(self, method: Callable, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        """
        This method calls the parent method of 'func'.
        ...
        :return: results indexed by data center
        """
        return {"local": self.__call_super_method(self, method.__name__, *args, **kwargs)}
    ...
    def query_file_hash(self, file_hash: str) -> Dict[str, Any]:
        ...
        if self.available_clients:
            rest = self.__call_everywhere(self.query_file_hash, file_hash)
        else:
            rest = self.__call_local(self.query_file_hash, file_hash)
        return tau_clients.merge_dicts(rest.values())

    def __init__(self, *args, **kwargs) -> None:
        """Constructor."""
        super(MultiDataCenterAnalysisClient, self).__init__(*args, **kwargs)
    ...
    def from_conf(  # pylint:disable=W0221
        ...
        :rtype: MultiDataCenterAnalysisClient
        :return: the initialized client
        :raises configparser.Error: any exception that can be raised by ConfigParser
        """
